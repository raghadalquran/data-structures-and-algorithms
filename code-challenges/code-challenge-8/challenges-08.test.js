...
The callback function to filter should include or
utilize a regular expression pattern.
...
const notInFirstArray = (forbiddenValues, arr) => {
  let arr2 = arr.filter(val =>{
    if(! (forbiddenValues.includes(val))) return true;
  })
  return arr2;
};
/* ------------------------------------------------------------------------------------------------
...
below, and an integer as input, uses filter to return
an array containing all stats with a baseStat greater than the integer.
...
getBaseStatGreaterThan function from challenge 4.
For this function, extend your solution from challenge 4 to only 
...
const getCharactersWithoutChildren = (arr) => {
  let arr2 = arr.filter(val=>{
    let kyes = Object.keys(val);
    if (!(kyes.includes('children'))){
      return true;
    }
  })
  return arr2;
...
Write a function named evenOddNumericValues that,
given an array as input,
uses filter to remove any non-numeric values,
then uses map to generate a new array containing the string 'even' or 'odd',
