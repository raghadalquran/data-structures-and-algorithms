...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach (val => {
    let first = val.substr(val.indexOf(' ') + 1);
    let sec =first.substr(first.indexOf(' ') + 1);
    result.push(sec);
  });
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach (val => {
    var sec = val.split(' ').slice(2).join(' ');
    result.push(sec);
  });
  return result;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach (val => {
    let firstWord = val.split(' ');
    result.push(firstWord[0]);
  });
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if(numberOfCharacters>strLength){
    return '';
