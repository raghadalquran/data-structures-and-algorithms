...
Write a function named forLoopTwoToThe that, given an array of integers as input, iterates over the array
 and returns a new array. The returned array should contain the result of raising 2 to the
 power of the original input element.
...
const extractAbilities = (arr) => {
  let arr2 =arr.map(val =>{
    let objVal = Object.values(val);
    //objVal [1,false,{url:'' , name : ''}]
    let name = Object.values(objVal[2]);
    //name [url , name ]
    return name[1];
  })
  return arr2;
...
const extractStats = (arr) => {
  let arr2 = arr.map((val) => {
    let objVal = Object.values(val);
    let sum = objVal[1]+objVal[2];
    let name1 = Object.values(objVal[0]);
    return {name:name1[1] , total:sum};
  });
  return arr2;
};
